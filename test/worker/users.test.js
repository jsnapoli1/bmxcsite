...
describe('users API', () => {
  ...
  it('creates a user with the requested permissions', async () => {
    ...
    const res = await call('POST', '/api/admin/users', {
      ...
      name: 'New Person',
      permissions: {
        blog: true, media: false, merch: false, campinfo: false, design: false, faces: false, registrations: false,
      },
    });
  ...
  it('leaves omitted permissions untouched on a partial PATCH', async () => {
    ...
    await call('POST', '/api/admin/users', {
      ...
      permissions: {
        blog: true, media: true, merch: true, campinfo: true, design: true, faces: true, registrations: true,
      },
    });
  ...
  it('clears a permission when it is explicitly set to false', async () => {
    ...
    const { user } = await res.json();
    expect(user.permissions).toEqual({
      blog: true, media: false, merch: true, campinfo: true, design: true, faces: true, registrations: true,
    });
  });
  ...
  it('still replaces every permission when a full object is sent', async () => {
    ...
    const res = await call('PATCH', '/api/admin/users/replaceall@example.com', {
      permissions: {
        blog: false, media: false, merch: false, campinfo: false, design: false, faces: false, registrations: false,
      },
    });
    expect(res.status).toBe(200);
    const { user } = await res.json();
